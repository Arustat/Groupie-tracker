...
func searchHandler(w http.ResponseWriter, r *http.Request){
	...
	log.Printf("Sort alpha value:", sortA)
    ...
    for _, index := range geoList.Index {
        for _, location := range index.Locations {
            ...
            if strings.Contains(strings.ToLower(locationavecspace), strings.ToLower(suggest)) {
                ville, pays := recupvilleetpays(location, suggestions, suggest)
				if ville != "" && pays != ""{
...
func recupvilleetpays(location string, suggestions []string, suggest string) (string, string) {
...
func filterDataByRelations(relationsData []struct {ID int `json:"id"`;DatesLocations map[string][]string `json:"datesLocations"`}, date, location, search string) ([]ArtistsInfo, error) {
